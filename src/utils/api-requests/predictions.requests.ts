...
export const getDailyPrediction = async (predictionTicker: string): Promise<any> => {
  try {
    const response = await fetch(`${import.meta.env.VITE_API_URL_PREDICTIONS}${import.meta.env.VITE_API_DAILY_PREDICTION}`)
...
export const getTwoWeekPredictions = async (predictionTicker: string): Promise<any> => {
  try {
    const response = await fetch(`${import.meta.env.VITE_API_URL_PREDICTIONS}${import.meta.env.VITE_API_TWO_WEEK_PREDICTIONS}`)
